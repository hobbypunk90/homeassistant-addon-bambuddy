...
- **🤖 AI failure detection** — Set `obico_ml_url` to point at a self-hosted [Obico ML server](https://github.com/TheSpaghettiDetective/obico-server) (`http://127.0.0.1:3333`). Native HA Add-on sidecar included in this repository. Push notifications include rich bounding boxes. MQTT payload includes `obico_active`, `obico_failed`, and `obico_monitoring` flags for resilient chamber light automations during pauses.
...
# Original Bambuddy Project Information

The following is the documentation from the upstream Bambuddy project.
